...
def test_pdf_node_basic():
    ...
    try:
        ...
        messages = [
            HumanMessage(
                content="이차방정식 x² + 2x + 1 = 0을 풀어주세요", metadata={"display": "content"}
            ),
            AIMessage(
                content="이 방정식은 완전제곱식입니다.\n\n1단계: 인수분해\nx² + 2x + 1 = (x + 1)² = 0\n\n2단계: 해 구하기\nx + 1 = 0\nx = -1 (중근)",
                metadata={"display": "content"},
            ),
        ]

        test_state = MockState(
            thread_id="test_thread_123", user_id="test_user_456", messages=messages
        )
        ...
        print("\nPDFRequest 생성 테스트...")
        pdf_request = asyncio.run(pdf_node._create_pdf_request_from_state(test_state))
        ...
        try:
            ...
            success = pdf_gen.generate_pdf_from_html_file(html_file, output_file)
        ...
        create_pdf_node(output_dir=str(temp_path))
