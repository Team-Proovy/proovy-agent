...
class MathPostProcessor:
    ...
    def _find_math_boundary(self, text: str, pos: int, direction: int) -> int:
        """수학 표현식의 경계를 찾음."""
        math_chars = set('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ()[]{}+-*/=<>^_.,\\')

        current_pos = pos
        while 0 <= current_pos < len(text):
            char = text[current_pos]
            if char in math_chars or ('가' <= char <= '힣'):
                current_pos += direction
            else:
                break

        return max(0, min(len(text), current_pos))
    ...
    def _merge_overlapping_regions(self, regions: list[tuple[int, int, str]]) -> list[tuple[int, int, str]]:
        ...
        for current in sorted_regions[1:]:
            ...
            if current[0] <= last[1] + 5:  # 5글자 이내 간격도 병합
                ...
                new_text = last[2] if len(last[2]) > len(current[2]) else current[2]
    ...
    def _convert_korean_terms(self, text: str) -> str:
        ...
        for korean_term, math_symbol in self.korean_math_terms.items():
    ...
    def extract_math_confidence(self, expression: MathExpression) -> float:
        ...
        # 괄호 균형 확인
        open_brackets = original_text.count('(') + original_text.count('[') + original_text.count('{')
        close_brackets = original_text.count(')') + original_text.count(']') + original_text.count('}')
