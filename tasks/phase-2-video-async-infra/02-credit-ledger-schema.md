...
- [x] ADR 0004 단일 원장 모델 재확인

## 구현 체크리스트

- [x] `credits` + `credit_holds` 테이블 (id/amount/status/expires_at/plan_id) + alembic
- [x] 가용잔액 쿼리 — `status='pending' AND expires_at > NOW()` 필터 (TTL-on-read)
- [x] 원자적 `hold` (SELECT FOR UPDATE, all-or-nothing) / `capture` (단일 row amount 부분차감) / `refund` (idempotent + `status != 'succeeded'` 가드)
- [x] 동시 요청 합산 + orphan hold 자동 회복 테스트

## Definition of Done

- [x] hold/capture/refund 원자성 테스트 통과
- [x] TTL-on-read orphan hold 자동 회복 테스트 (sweep cron 없음)
- [x] 자동화된 테스트 통과
