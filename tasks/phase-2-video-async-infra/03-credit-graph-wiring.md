...
- [ ] 비용 테이블(implementation_plan §9) 확인 (Flash 1 / Sonnet 3 / Opus 8 / code 1 / image 2 / video 10 / general 0.5)

## 구현 체크리스트

- [ ] Planner가 plan 전체 예상치 `hold` (부족 시 요청 통째 거절 = all-or-nothing)
- [ ] CreditSettler가 그래프 끝 동기작업(solve/pdf)분 `capture` finalize
- [ ] solve/pdf도 같은 balance에서 차감하도록 배선 (시스템 전반 단일 원장)
- [ ] 사용자 재시도 preflight (`retry_source_job_id` 검증 → hold 전, 실패 시 hold 안 만듦)

## Definition of Done

- [ ] plan hold → solve/pdf capture → 초과예상 환불 흐름 통합 테스트
- [ ] 가용 부족 시 요청 거절 ("N cr 필요")
- [ ] 자동화된 테스트 통과
