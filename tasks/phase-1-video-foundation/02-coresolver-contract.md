...
- [x] `code_generate` / `code_execute` 도구 존재 확인
- [x] `before_model` trim 미들웨어 위치 확인 (ADR 0006 — LLM 전달 시점만 trim, state 원본 유지)

## 구현 체크리스트

- [x] `core_solver/agent.py` Phase 1을 `create_agent(tools=[code_generate, code_execute], max_iter=5, before_model=trim)`로 구현
- [x] code/stdout을 `state.messages`에 **풀텍스트 persist** (ADR 0006 evidence)
- [x] verify 시스템 프롬프트가 마지막 메시지를 "단계+수식+답" 프로즈로 유도 + `kind="verified_solution"` 태깅
- [x] display 모드: full=hidden / brief=content (1.03과 연동)
- [x] (회귀 가드) 한 solve 후 messages에 `code_execute.args.code` + stdout ToolMessage + `kind="verified_solution"` AIMessage **셋 다** 존재 테스트

## Definition of Done

- [x] 회귀 가드 테스트 통과 (ADR 0006 evidence 3종 존재)
- [x] 자동화된 테스트 통과
- [x] max_iter 초과 시 현재 결과 반환 (무한 루프 없음)
