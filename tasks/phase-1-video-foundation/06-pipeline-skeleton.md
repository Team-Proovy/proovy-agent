...
- [ ] 설계 §3.6 워커 내부 구조(stage 명시 분해) / §5.1 VisualTypeRegistry 재확인

## 구현 체크리스트

- [ ] `VisualTypeRegistry` 통합 구조 — visual_type별 core 5필드(`schema`/`prompt_snippet`/`render_fn`/`fallback_candidates`/`narration_alignment_rule`)
- [ ] `StageContext` + orchestrator — `stage_solve/scriptify/tts/render/compose` 함수 분해 (checkpoint 없음)
- [ ] `stage_solve`: 주입된 SolutionPlan **검증만** (재-solve 없음)
- [ ] thin `stage_tts/render/compose` (실제 로직은 1.07/1.08/1.09)
- [ ] ffprobe 1초 fallback **제거**

## Definition of Done

- [ ] 빈 registry + StageContext로 orchestrator dry-run 통과 (stage 경계 테스트 seam 확인)
- [ ] 자동화된 테스트 통과
